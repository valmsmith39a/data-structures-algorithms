...
# sort sub array to sort whole array 

# O(n) time | O(n) space
def subarraySort(array):
	# single pass to get the min / max
...
def postOrderTraverse(tree, array):
	if tree is not None:
		...
		postOrderTraerse(tree.right, array)
		array.append(tree.value)
	return array
...
# iterative solution
# O(log(n)) time | O(1) space, bc don't store anything 
...
# recursive solution
#  O(log(n) time) | O(log(n)) space, because eliminating half the values with each iteration	
def binarySearchHelper(array, target, left, right):
	# left pointer crosses right pointer, didn't find the value
	if left > right:
		return -1 
	middle = (left + right) // 2
	potentialMatch = array[middle]
